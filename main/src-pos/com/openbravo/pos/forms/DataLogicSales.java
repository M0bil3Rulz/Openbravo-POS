...
public class DataLogicSales extends BeanFactoryDataSingle {
    ...
    public SentenceList getTicketsList() {
         return new StaticSentence(s
            , new QBFBuilder( //AK - added tableid and object
            "SELECT T.TICKETID, T.TABLEID, T.TICKETTYPE, R.DATENEW, P.NAME, C.NAME, SUM(PM.TOTAL) "+ 
            ...
            "WHERE ?(QBF_FILTER) GROUP BY T.ID, T.TICKETID, T.TABLEID, T.TICKETTYPE, R.DATENEW, P.NAME, C.NAME ORDER BY R.DATENEW DESC, T.TICKETID" 
            , new String[] { "T.TICKETID", 
                             "T.TABLEID", 
                             "T.TICKETTYPE", 
                             "PM.TOTAL", 
                             "R.DATENEW", 
    ...
    public final TicketInfo loadTicket(final int tickettype, final int ticketid) throws BasicException {
        TicketInfo ticket = (TicketInfo) new PreparedSentence(s
                , "SELECT T.ID, T.TICKETTYPE, T.TICKETID, T.TABLEID, R.DATENEW, R.MONEY, R.ATTRIBUTES, P.ID, P.NAME, T.CUSTOMER FROM RECEIPTS R JOIN TICKETS T ON R.ID = T.ID LEFT OUTER JOIN PEOPLE P ON T.PERSON = P.ID WHERE T.TICKETTYPE = ? AND T.TICKETID = ?"
    ...
    public final void saveTicket(final TicketInfo ticket, final String location) throws BasicException {
        ...
        Transaction t = new Transaction(s) {
            public Object transact() throws BasicException {
                ...
                // AK
                new PreparedSentence(s
                    , "INSERT INTO TICKETS (ID, TICKETTYPE, TICKETID, TABLEID, PERSON, CUSTOMER) VALUES (?, ?, ?, ?, ?, ?)"
                    , SerializerWriteParams.INSTANCE
                    ).exec(new DataParams() { public void writeValues() throws BasicException {
                        setString(1, ticket.getId());
                        setInt(2, ticket.getTicketType());
                        setInt(3, ticket.getTicketId());
                        setInt(4, ticket.getTableId()); // AK - increment numbers
